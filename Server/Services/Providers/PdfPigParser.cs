...
/// </summary>
public class PdfPigParser : IAdvancedDocumentParser
{
        ...
        try
        {
            ...
            foreach (Match match in emailMatches)
            {
                entities.Add(new ExtractedEntity(
                    ...
                    {
                        new EntityMention(match.Value, match.Index, match.Index + match.Length)
                    }
                ));
            }
    ...
    private bool IsLikelyTableRow(string line)
    {
    ...
    private TableData? ParseTableFromLines(List<string> lines)
    {
        ...
        try
        {
            ...
            for (int i = 1; i < lines.Count; i++)
            {
                ...
                if (rowLine.Contains('\t'))
                {
                    rowData = rowLine.Split('\t', StringSplitOptions.RemoveEmptyEntries)
                ...
                if (rowData.Count > headers.Count)
                    rowData = rowData.Take(headers.Count).ToList();
    ...
    private double CalculateTableConfidence(List<string> headers, List<List<string>> rows)
    {
